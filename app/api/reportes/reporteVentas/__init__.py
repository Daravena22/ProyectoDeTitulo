...
def generar_reporte():
    ...
    rows = db.session.query(Venta.id ,func.concat(Cliente.nombre, ' ', Cliente.apellido).label('cliente'),Venta.fecha,Venta.folio, Venta.monto_bruto.label('total'),Venta.total_abonado.label('abonado')).join(Cliente,Venta.cliente_id == Cliente.id)
    ...
    write_to_pdf(pdf, "Tabla con todas las ventas que se han realizado:")
    pdf.ln(15)
